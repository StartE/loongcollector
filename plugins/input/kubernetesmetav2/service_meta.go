...
func (s *ServiceK8sMeta) Start(collector pipeline.Collector) error {
	...
	s.metaCollector = &metaCollector{
		serviceK8sMeta:   s,
		collector:        collector,
		entityBuffer:     make(chan models.PipelineEvent, 5000), // Increase buffer size to 5000
		entityLinkBuffer: make(chan models.PipelineEvent, 5000), // Increase buffer size to 5000
